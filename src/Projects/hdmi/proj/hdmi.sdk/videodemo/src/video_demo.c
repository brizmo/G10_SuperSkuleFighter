...
/*				Global Variables								*/
/* ------------------------------------------------------------ */
volatile unsigned int * threshold = (unsigned int *)0x44A40000;
/*
...
/* ------------------------------------------------------------ */
void AddHealthBar(u8 *frame, u32 stride)
{
	u32 xcoi, ycoi;
	u32 iPixelAddr;
	u8 color = 255;
	...
	int starty = 50;
	int stopy = 70;
	for (xcoi = startx * 3 -1; xcoi < stopx * 3; xcoi +=3 ){
		iPixelAddr = starty * stride + xcoi;
		for (ycoi = starty; ycoi <= stopy; ycoi ++){
			frame[iPixelAddr] = 0;
			frame[iPixelAddr + 1] = color;
...
void DemoRun()
{
	...
	while (userInput != 'q')
	{
		...
		case '0':
			...
			while (1){
				/*for (i = 0; i < DISPLAY_NUM_FRAMES; i++){
					AddHealthBar(pFrames[i], DEMO_STRIDE);
				}*/

				AddHealthBar(pFrames[dispCtrl.curFrame], DEMO_STRIDE);
